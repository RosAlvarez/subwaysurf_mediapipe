...
def isLeftRight(results,frame,x_pos_index,game_started):
    if results.pose_landmarks:
        if game_started.value:
            frame, horizontal_position = checkLeftRight(frame, results, draw=True)
            if (horizontal_position=='Left' and x_pos_index.value!=0) or (horizontal_position=='Center' and x_pos_index.value==2):
                pyautogui.press('left')
                x_pos_index.value -= 1
            if (horizontal_position=='Right' and x_pos_index.value!=2) or (horizontal_position=='Center' and x_pos_index.value==0):
                pyautogui.press('right')
                x_pos_index.value += 1


def isJumpCrouch(results,frame,y_pos_index,game_started,MID_Y):
    if results.pose_landmarks:
        if MID_Y:
            if game_started.value:
                frame, posture = checkJumpCrouch(frame, results, MID_Y, draw=True)
                if posture == 'Crouching' and y_pos_index.value == 1:
                    pyautogui.press('down')
                    y_pos_index.value -= 1
                elif posture == 'Jumping' and y_pos_index.value == 1:
                    pyautogui.press('up')
                    y_pos_index.value += 1
                elif posture == 'Standing' and y_pos_index.value != 1:
                    y_pos_index.value = 1

def isHandsJoin(results,frame,MID_Y):
...
if __name__ == "__main__":
    ...
    #y_pos_index = 1
        ...
        ## global variables
    m=Manager()
    game_started = Value('b', False)
    x_pos_index = Value('i', 1)
    y_pos_index = Value('i', 1)
    
    MID_Y = None
    global_mid= m.Value(MID_Y)
    ...
    global_frame = m.Value(frame)

    frame_height, frame_width, _ = frame.shape
    frame, results = detectPose(frame, pose_video, draw=True)
    global_r=m.Value(results)

    p1=Process(target=isLeftRight,args=(global_r,global_frame,x_pos_index,game_started,global_mid))
    p2=Process(target=isJumpCrouch,args=(global_r,global_frame,y_pos_index,game_started,global_mid))
    p3=Process(target=isHandsJoin, args=(global_r,global_frame,global_mid))
